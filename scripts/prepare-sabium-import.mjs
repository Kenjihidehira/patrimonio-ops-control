...
const sheetArgument = process.argv[5];
const sourceSheet = sheetArgument && /^\d+$/.test(sheetArgument)
  ? Number(sheetArgument)
  : sheetArgument || 1;

if (!inputPath || !outputPath) {
  throw new Error(
    "Uso: node scripts/prepare-sabium-import.mjs <arquivo.xlsx> <saida.json> [tamanho-do-lote] [planilha]",
  );
}
...
const sourceRows = isJsonInput
  ...
  : await readXlsxFile(resolvedInput, { sheet: sourceSheet });
...
const preparedRows = rows.slice(1).map((row, index) => {
  ...
  if (!/^\d{1,10}(?:\.\d{1,6})?$/.test(sourceIdentifier)) {
    throw new Error(`Linha ${sourceRow}: identificador Sabium inválido.`);
  }
