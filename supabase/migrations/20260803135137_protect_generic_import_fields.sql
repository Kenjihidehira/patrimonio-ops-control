...
create or replace function public.patrimonio_import_assets_without_aliases(
  p_owner_key text,
  p_actor text,
...
begin
  ...
  set
    ...
    location = case
      when excluded.location = 'Não informada na planilha'
        then patrimonio_assets.location
      else excluded.location
    end,
    serial = case
      when excluded.serial = '' then patrimonio_assets.serial
      else excluded.serial
    end,
    brand_model = case
      when excluded.brand_model = 'Não informado na planilha'
        then patrimonio_assets.brand_model
      else excluded.brand_model
    end,
    acquired_at = patrimonio_assets.acquired_at,
    acquisition_value = patrimonio_assets.acquisition_value,
    status = excluded.status,
    notes = case
      when excluded.notes = 'Importado da planilha de patrimônios.'
        then patrimonio_assets.notes
      else excluded.notes
    end,
    updated_at = now();
  ...
  select
    ...
    concat_ws(
      ...
      persisted_asset.location,
      coalesce(nullif(persisted_asset.assignee, ''), 'Sem responsável')
    ),
  ...
  join public.patrimonio_nuclei persisted_nucleus
    on persisted_nucleus.owner_key = p_owner_key
    and persisted_nucleus.code = upper(source_nucleus.code)
  join public.patrimonio_assets persisted_asset
    on persisted_asset.owner_key = p_owner_key
    and persisted_asset.code = row.code;
