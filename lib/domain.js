...
export function buildDashboard(input, filters = {}) {
  ...
  const collaborators = state.collaborators.map((collaborator) => {
    const assignedAssets = assetsByAssignee.get(normalizedText(collaborator.name)) ?? [];
    return {
      ...collaborator,
      nucleus: nucleusMap.get(collaborator.nucleusId),
  ...
  const summary = {
    ...
    collaboratorsWithoutAssets: collaborators.filter((collaborator) => !collaborator.hasAssets).length,
  };
