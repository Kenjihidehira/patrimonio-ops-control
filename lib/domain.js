...
const sabiumSourceIdentifierPattern = /^\d{1,10}(?:\.\d{1,6})?$/;
...
export function normalizeState(input) {
  ...
  const assetAliases = Array.isArray(input.assetAliases)
    ? input.assetAliases
      .map((alias) => ({
        sourceCode: optionalText(alias?.sourceCode),
        assetId: optionalText(alias?.assetId),
      }))
      .filter((alias) => alias.sourceCode && assetIds.has(alias.assetId))
    : [];
...
export function buildDashboard(input, filters = {}, access = {}) {
  const state = normalizeState(input);
  const includeFinancials = access.includeFinancials === true;
  ...
  const enriched = state.assets.map((asset) => {
    ...
    delete visibleAsset.sourceFingerprint;
    delete visibleAsset.sourceRow;
    if (!includeFinancials) {
      visibleAsset.value = null;
      visibleAsset.operationValue = null;
      visibleAsset.invoiceNumber = "";
    }
    return {
      ...visibleAsset,
...
function updateAssetIdentifier(state, action, actor, at, movementId) {
  ...
  if (asset.sourceSystem === "sabium") {
    throw new DomainError("Identificadores vindos do Sabium não podem ser alterados manualmente.");
  }
...
function updateAssetDetails(state, action, actor, at, movementId) {
  ...
  if (
    asset.sourceSystem !== "sabium"
    && !isAssetIdentifierValidForType(asset.id, next.type)
    && !(next.type !== "fleet" && untaggedAssetIdPattern.test(asset.id))
  ) {
    throw new DomainError("O tipo selecionado não é compatível com o identificador atual.");
  }
  const fieldLabels = {
    type: "tipo",
...
function normalizeAsset(input, nucleusIds) {
  ...
  const baseCode = optionalText(input.baseCode);
  const incorporation = optionalInteger(input.incorporation);
  const sourceDescription = optionalText(input.sourceDescription);
  ...
  if (
    ...
    && (
      ...
      || !baseCode
      || incorporation === null
      || !sabiumSourceIdentifierPattern.test(sourceIdentifier)
      || !sourceDescription
      || sourceRow === null
      || sourceRow < 2
    )
