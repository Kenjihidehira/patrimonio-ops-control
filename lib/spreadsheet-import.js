...
const typeAliases = new Map([
  ...
  ["veiculo", "fleet"],
  ["automovel", "car"],
  ["implemento rodoviario", "trailer"],
  ["componente de frota", "vehicle_component"],
  ["maquina ou equipamento", "equipment"],
  ["movel ou utensilio", "furniture"],
...
function parseFlatRows(rows, header) {
  ...
  for (let rowIndex = header.rowIndex + 1; rowIndex < rows.length; rowIndex += 1) {
    ...
    const result = normalizeAssetCode(codeValue, type);
    ...
    if (result.kind === "untagged" && type === "fleet") {
      assets.push({
        invalid: true,
        code: "Sem patrimônio",
        rowIndex,
        columnIndex: header.columns.code,
        issue: "Toda frota precisa ter número; o patrimônio será número-da-frota.0.",
      });
      continue;
    }
    if (result.kind === "invalid") {
      assets.push({
        ...
        columnIndex: header.columns.code,
        issue: type === "fleet"
          ? "A frota deve conter somente números; o patrimônio será número-da-frota.0."
          : "O identificador deve conter 5 ou 6 números.",
      });
      continue;
    }
    if (!type || !nucleus) {
...
function normalizeAssetCode(value, type = null) {
  const rawCode = normalizeCell(value);
  const key = normalizeKey(rawCode);
  if (!rawCode || key === "x") return { kind: "empty", code: "" };
  if (["sem patrimonio", "sem patrimonios"].includes(key)) {
    return { kind: "untagged", code: "" };
  }
  if (key === "pessoal") return { kind: "personal", code: rawCode };
  if (isUntaggedAssetCode(rawCode)) {
    return { kind: "untagged", code: rawCode.toUpperCase() };
  }
  if (type === "fleet") {
    if (/^\d{1,10}\.0$/.test(rawCode)) {
      return { kind: "valid", code: rawCode, original: rawCode, padded: false };
    }
    ...
    return { kind: "invalid", code: rawCode };
  }
